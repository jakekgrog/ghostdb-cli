...
func createNode(ch chan<- Data, name, http, raft, join string) {
	if join == "" {
		cmd := exec.Command("C:\\Users\\Jake\\Desktop\\ghostdb\\GhostDB\\ghostdb.exe", "-id", name, "-http", http, "-raft", raft, "./"+name, "&")
		err := cmd.Start()
		ch <- Data {
			error: err,
		}
	} else {
		cmd := exec.Command("C:\\Users\\Jake\\Desktop\\ghostdb\\GhostDB\\ghostdb.exe", "-id", name, "-http", http, "-raft", raft, "-join", join, "./"+name, "&")
		err := cmd.Start()
		ch <- Data {
			error: err,
		}
